...
CREATE TABLE merchandise_t(
	...
	CONSTRAINT TypeOfKit_chk CHECK (TypeOfKit IN ('Home', 'Away', 'Third Kit')),
...
BEGIN
    ...
    club_max AS (
        SELECT sd.ClubID, sd.PIDNumber, sd.SportNetSpend AS LargestPurchase
        FROM sportingdirector_t sd
        JOIN (
        ...
        ON sd.ClubID = mx.ClubID AND sd.SportNetSpend = mx.MaxSpend --joining the tables
    )
    SELECT 
