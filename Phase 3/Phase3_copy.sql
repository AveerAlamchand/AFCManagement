...
DELIMITER $$
CREATE PROCEDURE UpdateStore(IN in_StoreID VARCHAR(8),IN in_StoreNetIncome FLOAT,IN in_StoreName VARCHAR(20),IN in_YearEstablished DATE,IN in_UnitOfBuilding VARCHAR(2),IN in_Manager VARCHAR(20),IN in_NumberOfStaff INT,IN in_Month VARCHAR(9),IN in_Year INT,IN in_StadiumID VARCHAR(8))
BEGIN
    UPDATE STORES
    SET 
    ...
    WHERE 
        StoreID = in_StoreID ANDStoreNetIncome = in_StoreNetIncome;
END $$
DELIMITER ;
